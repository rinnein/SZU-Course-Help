...
- 课程目录增加进程级请求节奏控制，默认在相邻学校分页请求间等待 600ms；多个浏览器请求也无法绕过该限制。
- 新增按账号摘要、批次、校区、课程类型和分页参数严格隔离的本地只读缓存，学校未开放或临时异常时可手动查看最近一次成功课程。
- 缓存仅接收成功且非空的完整页面，采用原子写入，并限制有效期、最长保留时间和总条目数；页面会标注缓存时间与过期状态。
- 新增“隐藏冲突”和“隐藏已满”筛选；已经选择的教学班始终可见，课程仍默认折叠。
- 学校明确限流时执行有限次数指数退避，并显示可恢复提示；普通业务拒绝不会再被误当作限流。
- 启动工作台前执行 OCR 兼容性自检；依赖不可用时保留手动登录，并明确说明自动重登录不可用的原因。
- 本地 API 增加同源保护；“学校官网”通过系统浏览器直接打开，不代理学校页面，也不在助手页面中执行学校脚本。
- CI 同时验证 Python 3.13 和 3.14，Release 构建继续真实初始化 OCR 引擎；发布区只提供 ZIP，不附带独立 SHA256 文件。
- 保留 `v3.4.0` 的多校区、完整目录搜索、只读课表、200 次未知响应保护、暂停/继续及可见 OCR 自动重登录逻辑。
...
| 课程筛选与只读缓存 | 可隐藏冲突或已满教学班；非开放期可显式查看当前账号范围内的本地课程缓存 |
...
“隐藏冲突”和“隐藏已满”仅改变当前浏览器中的展示，已经选中的教学班始终保留可见。全目录搜索按顺序读取分页，浏览器和后端共同限制请求节奏；只有学校明确提示限流时才执行有限次数的退避重试，普通业务拒绝不会被自动重试。

成功读取的非空课程页会写入只读缓存，范围同时包含学号摘要、批次、校区、课程类型、页码和固定页大小。缓存模式必须手动开启，页面会显示缓存时间和是否过期；缓存教学班不能加入清单。课程数据超过 7 天后不再返回，不同账号、批次或校区之间不会回退复用。
...
| `COURSE_SELECT_CATALOG_PAGE_DELAY_MS` | `600` | 全目录相邻学校请求的最小间隔（毫秒） |
| `COURSE_SELECT_CATALOG_THROTTLE_RETRIES` | `3` | 学校明确限流时的前端重试上限 |
| `COURSE_SELECT_CATALOG_THROTTLE_BACKOFF_MS` | `2000` | 首次限流退避时间（毫秒） |
| `COURSE_SELECT_CATALOG_CACHE_TTL_SECONDS` | `21600` | 课程目录缓存的新鲜期限（秒） |
...
请先升级到 v3.5.0 或更新版本；该版本兼容 `ddddocr 1.6.1` 的多种导出布局，并会在启动工作台以及各平台 Release 构建前真实初始化 OCR 引擎。源码运行建议使用 Python 3.13，CI 同时验证 Python 3.14。请重新安装 `requirements.txt`，不要复用依赖不明的虚拟环境。

### 什么时候可以使用课程缓存？

成功读取过课程页面后，可以打开“使用缓存”查看同一学号、批次、校区、课程类别与页码的只读副本。缓存不会跨账号、批次或校区复用，也不能从缓存加入选课清单。学校关闭课程查询时仍可查看已有缓存；没有匹配缓存时页面会明确提示，不会回退到其他账号或校区的数据。
